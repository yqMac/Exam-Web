            ...
            $.ajax({
                ...
                success: function (msg) {
                    if ($.trim(msg) != "") {
                        ...
                        $("#selQuesPoint").html("<option value=''>该题库无知识点</option>");
                    }
                }
            });
        }
...
<sf:form name="form1" method="post" id="form1" modelAttribute="TQues">
    ...
    <div class="navigation">
        <a href="javascript:history.go(-1);" class="back">后退</a>首页 &gt; <a
            href="/ques/queses">题目管理</a> &gt; <a href="#">修改题目</a></div>
    <div>
        <table class="form_table">
            ...
            <tr>
                <th>题库集：</th>
                <td>
                    <sf:select path="TQuesBank.id" onclick="bankChange()" class="select2 required" id="selQuesBank"
                               items="${banks}" itemLabel="bankName" itemValue="id">

                    </sf:select>
                        <%--<input type="text" name="bankId" value="${bankId}"/>--%>
                        <%--<select  name="bankId" id="selQuesBank" onclick="bankChange()" class="select2 required">--%>
                        <%--<option >==选择题库==</option>--%>
            ...
            <tr>
                <th>题库：</th>
                <td>
                    <sf:select path="TQuesLib.id" items="${libs}" itemLabel="libName" itemValue="id" id="selQuesLib" onclick="libChange()" class="select2 required"/>
                </td>
            </tr>
            <tr>
                <th>知识点：</th>
                <td>
                    <sf:select path="TQuesPoint.id" items="${points}" itemValue="id" itemLabel="pointName" id="selQuesPoint" class="select2 required"/>
                </td>
            </tr>
            <tr>
                <th>题目类型：</th>
                <td>
                    <sf:select path="TQuesType.id" items="${qtypes}" itemLabel="typeName" itemValue="id" id="selQuesType" class="select2 required">
                    </sf:select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <table border="0" cellspacing="0" cellpadding="0" class="border_table">
                        ...
                        <tbody id="nav_box">
                        <c:forEach items="${optiontexts}" var="op">
                            <tr class="td_c">
                                <td><input name="optiontexts" type="text" value="${op}" class="txtInput middle"/></td>
                                <td><img alt="删除" src="<%=path%>/html/images/icon_del.gif" class="operator"/></td>
                            </tr>
                        </c:forEach>
